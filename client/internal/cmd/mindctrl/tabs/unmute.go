...
func init() {
	UnmuteCommand.Args = func(cmd *cobra.Command, args []string) error {
		length := len(args)
	...
	UnmuteCommand.RunE = func(cmd *cobra.Command, args []string) error {
		if transport, err := options.GetTransport(cmd); err != nil {
			...
			if data, err := operation.Execute(transport); err != nil {
				return errors.WrapExecutionError(err, "cannot unmute tab %d", tab)
			} else {
				printOperationResult(stdout, tab, "unmuted", data)
				return nil
			}
		}
	}
