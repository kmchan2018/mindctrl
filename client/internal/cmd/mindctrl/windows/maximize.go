...
var (
	MaximizeCommand = &cobra.Command{
		Use:   "maximize [ window ]",
		Short: "Maximize the target window",
		Long:  "Maximize the target window",
...
func init() {
	MaximizeCommand.Args = func(cmd *cobra.Command, args []string) error {
		length := len(args)
	...
	MaximizeCommand.RunE = func(cmd *cobra.Command, args []string) error {
		if transport, err := options.GetTransport(cmd); err != nil {
			...
			if data, err := operation.Execute(transport); err != nil {
				return errors.WrapExecutionError(err, "cannot maximize window %d", window)
			} else {
				printOperationResult(stdout, window, "maximized", data)
				return nil
			}
		}
	}
