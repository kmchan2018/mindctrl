...
func init() {
	CancelCommand.Args = func(cmd *cobra.Command, args []string) error {
		length := len(args)
	...
	CancelCommand.RunE = func(cmd *cobra.Command, args []string) error {
		if transport, err := options.GetTransport(cmd); err != nil {
			...
			operation := mindctrl.CancelDownload(download)
			stdout := cmd.OutOrStdout()

			if data, err := operation.Execute(transport); err != nil {
				return errors.WrapExecutionError(err, "cannot cancel download")
			} else {
				printOperationResult(stdout, download, "cancelled", data)
				return nil
			}
		}
	}
